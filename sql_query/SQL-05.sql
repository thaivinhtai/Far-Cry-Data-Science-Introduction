...
select match_id, player_name, sum(kill_count) as kill_count,
       sum(death_count) as death_count, sum(suicide_count) as suicide_count,
       round(sum(kill_count) * 100.0 / (sum(kill_count) + sum(death_count) +
             sum(suicide_count)), 2) as efficiency
from (
      select match_id, player_name, sum(kill_count) as kill_count,
...
group by match_id, player_name order by match_id, efficiency desc;
